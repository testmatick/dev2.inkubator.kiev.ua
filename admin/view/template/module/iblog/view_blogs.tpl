...
$(document).ready(function(){
	...
	category = $('#input-sort option:selected').val();
	 $.ajax({
		url: "index.php?route=module/<?php echo $moduleNameSmall; ?>/getPosts&category="+category+"&token=<?php echo $token; ?>&page=1&store_id=<?php echo $store_id; ?>",
		type: 'get',
		dataType: 'html',
		success: function(data) {
			$("#PostsWrapper<?php echo $store_id; ?>").html(data);
